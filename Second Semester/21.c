...
void print(netdata net[20], int same[20], int sum) {
	printf("machines ");
	int i;
	for (i = 0; i < sum - 2; i++)
		printf("%s, ", net[same[i]].name);
	printf("%s and %s are on the same local network.\n", net[same[i]].name, net[same[i + 1]].name);
}
